...
export const authOptions : any = {
    providers : [
        ...
        CredentialsProvider({
            ...
            //   로그인 요청 시 실행되는 코드 디비와 비교 이후 맞으면 return
            // user 정보를 보내고 틀리면 return null
              async authorize(credentials): Promise<User | null> {
                try{
                    
                const [results] = await db.query<RowDataPacket[]>('select * from choiyul.member where email = ?', [credentials?.email]);
